...
index do
  ...
  actions defaults: false do |event|
    ...
    item "编辑", "#{edit_cpanel_event_path(event)}?owner_type=#{event.eventable.try(:class)}&owner_id=#{event.eventable.try(:id)}"
...
form html: { multipart: true } do |f|
  ...
  f.inputs do
    ...
    f.input :eventable_type, as: :hidden, input_html: { value: "#{params[:owner_type]}" }
    f.input :eventable_id,   as: :hidden, input_html: { value: "#{params[:owner_id]}" }
  end
